...
const TypewriterText = ({ text }) => {
  ...
  return <span>{displayedText}{isTyping && <span className="typewriter-cursor">|</span>}</span>;
};
...
function HeroSection({ data }) {
  ...
  const animateCardExpansion = useCallback((direction) => {
    ...
    const heroRect = bgOverlayRef.current.parentElement.getBoundingClientRect();
    ...
    gsap.set(clone, {
      ...
      width: rect.width,
      height: rect.height,
      backgroundImage: `linear-gradient(to left, rgba(10, 15, 12, 0.795), rgba(0, 122, 94, 0.85)), url(${nextBgImage})`,
      opacity: 1,
      scale: 1,
      x: 0,
      y: 0,
    ...
    masterTL.call(() => {
      if (bgOverlay) bgOverlay.style.backgroundImage = `linear-gradient(to left, rgba(10, 15, 12, 0.795), rgba(0, 122, 94, 0.85)), url(${nextBgImage})`;
    }, null, 0.6);
  ...
  return (
    <section className="hero-section">
      <div
        ...
        style={{ backgroundImage: `linear-gradient(to left, rgba(10, 15, 12, 0.795), rgba(0, 122, 94, 0.85)), url(${getImgUrl(services[currentBgIndex]?.image)})` }}
      />
        ...
        <div className="hero-content">
          <div className="text-box">
            <h1 ref={titleRef} className="hero-title">
              <TypewriterText key={displayTextIndex} text={services[displayTextIndex]?.title || ""} />
            </h1>
